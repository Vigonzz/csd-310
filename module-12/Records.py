...
# We need to retrive column names from our distributor tableto be returned as a list of column names
...
# Create a table for distributr and wine sales 
def fetch_distributor_wine_sales(connection, distributor_name_column):
    # Use a SQL query to fetch the delivery information, additionally it joins deleivery, orders, 
    # and wine tables to continue gathering information
    try:
        cursor = connection.cursor()
        query = f"""
        SELECT DISTINCT 
        ...
        if not results:
            ...
            print("-" * 50)
            # allow for the width of the columns to become flexable
            ...
            sold_col = max(len('Wines Sold Per Bottle'), max(len(str(row[2])) for row in results))

            print(f"{'Distributor':<{distributor_col + 2}} {'Wine':<{wine_col + 2}} {'Wines Sold Per Bottle':<{sold_col + 2}}")
...
#Use error handling to take into account different occurances
    except Error as e:
        print(f"Error: '{e}' occurred while fetching the distributor-wine relationships")

# Create a table focused on customer delivery 
def fetch_customer_delivery(connection, distributor_name_column):
    # Use SQL query to gather the customer delivery data and join the tables together 
    try:
        cursor = connection.cursor()
        query = f"""
...
# Create a table for employee hours
def fetch_employee_hours(connection):
    # Use an SQL query to collect employee hours 
    try:
        cursor = connection.cursor()
        query = """
...
def main():
    ...
    if connection:
        # if the connection works gather the distributor column names and call the three data retrival functions
